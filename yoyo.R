...
eGFR_change_df <- read.csv('eGFR_change_df_1th.csv', encoding = 'euc-kr') # baseline 1기
eGFR_change2_df <- read.csv('eGFR_change2_df_5th.csv', encoding = 'euc-kr') # baseline 5기
# CKD_df <- read.csv('CKD_df.csv', encoding = 'euc-kr')  # 제외
FM_change1_df <- read.csv('FM_change1_df_1th.csv', encoding = 'euc-kr') # baseline 1기
FM_change2_df <- read.csv('FM_change2_df_1th.csv', encoding = 'euc-kr') # baseline 1기
FM_change3_df <- read.csv('FM_change3_df_1th.csv', encoding = 'euc-kr') # baseline 1기

CKD_df <- read.csv('CKD_df_5th.csv', encoding = 'euc-kr') # baseline 5기
...
df_selection <- subset(df_selection, select=-c(기수, EDATE, NIHID))
...
#### Adjusted Fit
  ...
  all_variables <- c(all.vars(formula(step_lm)[[3]])) # stepwise로 선택된 변수들만 가져오기
  all_variables <- all_variables[!all_variables %in% c('yoyo_03', 'yoyo_05', 'yoyo_07', 'yoyo_10')] # yoyo 변수들 선택되었을 경우 빼주기
  ...
  if ('yoyo_03' %in% all_variables == FALSE) {  # yoyo_03 선택 안됐을 경우에만
  ...
  if ('yoyo_05' %in% all_variables == FALSE) {  # yoyo_05 선택 안됐을 경우에만
  ...
  if ('yoyo_07' %in% all_variables == FALSE) {  # yoyo_07 선택 안됐을 경우에만
  ...
  if ('yoyo_10' %in% all_variables == FALSE) {  # yoyo_10 선택 안됐을 경우에만
